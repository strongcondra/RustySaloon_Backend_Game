...
const Game = ({ game, user, isAuthenticated }) => {
  ...
  const joinGame = color => {
    ...
    if (game.privateGame) {
      ...
      setJoining(color);
      socket.emit("join-game", game._id, color);
    }
  };
  ...
  const captchaOnChange = response => {
    ...
    socket.emit("join-game", game._id, selectedCup, response);
  };
  ...
  useEffect(() => {
    ...
    // Listeners
    socket.on("game-join-error", joinEvent);
    socket.on("game-join-success", joinEvent);
    socket.on("game-call-bot-error", callingBotEvent);
    socket.on("game-call-bot-success", callingBotEvent);
